...
function pollStatistics(currentJob, interval, lastTimestamp, fromArchiv) {
  $('.chart-loading-icon').fadeIn();
  var params = { get: "customStatistics", job: currentJob, startTimestamp: lastTimestamp };

  if (fromArchiv) {
    params = { get: "job", job: currentJob };
  }

  $.getJSON('/jobsInfo', params)
    .done(function(json) {
      var nextTimestamp = Infinity;
      
      if (fromArchiv) {
        json = json[0].customStatistic.data;
      }

      // updateJobOverviewList(jobId, json.jobs[jobId], currentJob);
      nextTimestamp = Math.min(nextTimestamp, updateJob(currentJob, json, json.refreshInterval, json.maxEntriesCount));
      ...
      if (!fromArchiv)
        setTimeout(pollStatistics, json.refreshInterval, currentJob, json.refreshInterval, nextTimestamp, false)
    })
    .error(function() {
      setTimeout(pollStatistics, interval, jobId, interval, fromArchiv);
    })
}
...
function startJobPolling() {
  ...
  $.getJSON('/jobsInfo', { get: 'archive' })
    .done(function(json) {
      ...
      if (found != null) {
        pollStatistics(currentJob, 1000, null, true);

      } else {
        // 2. otherwise poll
        pollStatistics(currentJob, 1000, null, false);
      }
    })
    .error(function() {
      alert("Failure while fetching json archive...");
    });
}
