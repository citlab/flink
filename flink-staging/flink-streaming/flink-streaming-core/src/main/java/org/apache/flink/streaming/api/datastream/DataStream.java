...
import org.apache.flink.streaming.api.constraint.identifier.AutoGeneratedConstraintIdentifier;
...
public class DataStream<OUT> {
	 ...
	 * window.
	 * <p>
	 * Call {@link StreamCrossOperator.CrossWindow#with(org.apache.flink.api.common.functions.CrossFunction)}} to
	 ...
	 * {@link StreamJoinOperator.JoinedStream#with(org.apache.flink.api.common.functions.JoinFunction)} to apply
	...
	/**
	 * Marks the start of a latency constraint.
	 *
	 * @param maxLatency
	 * 		the desired maximum latency.
	 * @return The {@link DataStream}.
	 */
	public DataStream<OUT> beginLatencyConstraint(long maxLatency) {
		return beginLatencyConstraint(new AutoGeneratedConstraintIdentifier(), maxLatency);
	}

	/**
	 * Marks the start of a latency constraint with the given name.
	 *
	 * @param name
	 ...
	 * @return The {@link DataStream}.
	 */
	public DataStream<OUT> beginLatencyConstraint(String name, long maxLatency) {
		return beginLatencyConstraint(new NamedConstraintIdentifier(name), maxLatency);
	}

	private DataStream<OUT> beginLatencyConstraint(ConstraintIdentifier identifier, long maxLatency) {
		streamGraph.beginLatencyConstraint(getId(), identifier, maxLatency);
		return this;
	}

	/**
	 * Mark the end of the last opened latency constraint.
	 *
	 * @return The {@link DataStream}.
	 */
	...
	/**
	 * Define a latency constraint between this object (begin) and the given DataStream (end).
	 *
	 * @param stream
	 * 		the given DataStream marking the end of the constraint.
	 * @param maxLatency
	 * 		the desired maximum latency.
	 */
	public void constrainLatencyBetween(DataStream<?> stream, long maxLatency) {
		constrainLatencyBetween(stream, maxLatency, new AutoGeneratedConstraintIdentifier());
	}

	/**
	 * Define a named latency constraint between this object (begin) and the given DataStream (end).
	 *
	 * @param stream
	 * 		the given DataStream marking the end of the constraint.
	 * @param maxLatency
	 * 		the desired maximum latency.
	 */
	public void constrainLatencyBetween(DataStream<?> stream, long maxLatency, String name) {
		constrainLatencyBetween(stream, maxLatency, new NamedConstraintIdentifier(name));
	}

	private void constrainLatencyBetween(DataStream<?> stream, long maxLatency, ConstraintIdentifier identifier) {
		constrainLatencyBetween(getId(), stream.getId(), maxLatency, identifier);
	}

	private void constrainLatencyBetween(int beginId, int endId, long maxLatency, ConstraintIdentifier identifier) {
		streamGraph.constrainLatencyBetween(beginId, endId, maxLatency, identifier);
	}
}
